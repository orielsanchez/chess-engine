...
fn test_search_terminates_early_with_tablebase_mate() {
    ...
    assert!(search_with_tb.evaluation > 19000, "With tablebase should return mate score ~19900, got {}", search_with_tb.evaluation);
    
    // Without tablebase should return heuristic evaluation
    assert!(search_without_tb.evaluation < 10000, "Without tablebase should return heuristic score, got {}", search_without_tb.evaluation);
...
fn test_search_returns_tablebase_result_immediately() {
    ...
    assert!(search_result.evaluation > 19000, "Should return tablebase mate score, got {}", search_result.evaluation);
    assert!(search_result.evaluation < 20000, "Should return tablebase mate score, got {}", search_result.evaluation);
}
...
fn test_search_prioritizes_tablebase_winning_moves() {
    ...
    assert!(search_result.evaluation > 19000, "Should find tablebase mate, got {}", search_result.evaluation);
}
...
fn test_search_handles_mixed_tablebase_positions() {
    ...
    assert!(search_result.evaluation.abs() > 100, "Should find significant advantage");
}
...
fn test_search_respects_dtz_fifty_move_rule() {
    ...
    assert!(search_result.evaluation > 0, "Should find winning move that avoids 50-move draw");
}
...
fn test_search_performance_with_tablebase_integration() {
    ...
    let position = Position::from_fen("rnbqkbnr/pppppppp/8/8/4P3/8/PPPP1PPP/RNBQKBNR b KQkq e3 0 1").unwrap();
    ...
    assert!(result.is_ok());
    assert!(elapsed < std::time::Duration::from_millis(1100), "Search should not be significantly slower");
}
...
fn test_search_node_count_optimization_with_tablebase() {
    ...
    assert!(search_result.nodes_searched < 10000, "Should search fewer nodes with tablebase");
}